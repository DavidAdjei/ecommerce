    ...
    return (
        <tr className="cart-item">
            ...
            <td className="item-brand">
                <p>{item.product.Brand ? item.product.Brand : item.product.Genre ? item.product.Genre : item.product.For ? item.product.For : item.product.Type}</p>
            </td>
            ...
            <td className='remove'> 
                <button onClick={() => removeFromCart(item.product._id)} className="remove-item">×</button>
            </td>
        </tr>
    );
};
...
export default connect(mapStateToProps, {increment, decrement, removeFromCart})(CartItem);
